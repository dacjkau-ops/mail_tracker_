...
must_haves:
  truths:
    ...
    - "multi_assign is blocked for auditor and clerk (only AG/DAG allowed by views.py line 637 guard)"
    - "reopen is blocked for auditor and clerk (only AG allowed by views.py line 518 guard)"
    - "close of multi-assigned mails is blocked for auditor and clerk (only AG allowed by views.py line 460 guard)"
  artifacts:
  ...
  key_links:
    ...
    - from: "backend/records/views.py multi_assign()"
      to: "auditor/clerk block"
      via: "role not in ['AG', 'DAG'] guard at line ~637"
      pattern: "role not in.*AG.*DAG"
    - from: "backend/records/views.py reopen()"
      to: "auditor/clerk block"
      via: "role != 'AG' guard at line ~518"
      pattern: "role != 'AG'"
    - from: "backend/records/views.py close()"
      to: "multi-assigned close block for auditor/clerk"
      via: "is_multi_assigned guard: role != 'AG' at line ~460"
      pattern: "is_multi_assigned.*role != 'AG'"
---
...
**IMPORTANT — Expanding `create` does NOT grant auditor/clerk access to multi_assign, reopen, or close-of-multi-assigned.** These three actions are guarded independently in the view body itself:
- `multi_assign()`: has an explicit `if user.role not in ['AG', 'DAG']: return 403` guard (views.py ~line 637). Auditor and clerk are blocked here regardless of has_permission() outcome.
- `reopen()`: has an explicit `if request.user.role != 'AG': return 403` guard (views.py ~line 518). Auditor and clerk are blocked here.
- `close()` for multi-assigned mails: has `if mail_record.is_multi_assigned and user.role != 'AG': return 403` guard (views.py ~line 460). Auditor and clerk are blocked here.
- `close()` for single-assigned mails: checks `mail_record.current_handler != user`, so auditor/clerk can close if they are the current handler — this is the intended behavior per the phase context.

No changes needed to these guards. Document their existence here so the executor can verify they remain in place.
...
src = open('config/permissions.py').read()
print('_can_view_mail has clerk:', 'clerk' in src)
print('_can_view_mail has auditor:', 'auditor' in src)

# Verify existing guards remain in views.py (should not have been touched here)
views_src = open('records/views.py').read()
print('multi_assign AG/DAG guard present:', \"role not in ['AG', 'DAG']\" in views_src)
print('reopen AG-only guard present:', \"role != 'AG'\" in views_src)
"
```
Expected: MailRecordPermission loads without ImportError, 'clerk' and 'auditor' strings present in permissions.py, both view guards confirmed present.
  </verify>
  <done>
...
Existing view guards confirmed intact:
- multi_assign blocked for auditor/clerk: views.py guard `if user.role not in ['AG', 'DAG']: return 403`
- reopen blocked for auditor/clerk: views.py guard `if request.user.role != 'AG': return 403`
- close of multi-assigned blocked for auditor/clerk: views.py guard `if mail_record.is_multi_assigned and user.role != 'AG': return 403`
  </done>
</task>
...
**DO NOT modify** the multi_assign(), reopen(), or close() method bodies — their existing role guards correctly block auditor/clerk and must remain unchanged.
  </action>
  <verify>
```bash
cd D:/Office/2026/Mail_Tracker/backend && python manage.py check && python -c "
...
print('Auditor SrAO restriction in reassign:', 'Auditors can only reassign to SrAO or AAO' in src)

# Confirm existing guards still present
print('multi_assign guard intact:', \"role not in ['AG', 'DAG']\" in src)
print('reopen guard intact:', \"role != 'AG'\" in src)
"
```
Expected: ViewSet loads OK, clerk/auditor branches present, subsection filter present, auditor SrAO restriction in reassign, both original guards confirmed still present.
  </verify>
  <done>
...
Existing guards confirmed untouched:
- multi_assign: `if user.role not in ['AG', 'DAG']: return 403` — auditor/clerk blocked
- reopen: `if request.user.role != 'AG': return 403` — auditor/clerk blocked
- close of multi-assigned: `if mail_record.is_multi_assigned and user.role != 'AG': return 403` — auditor/clerk blocked
  </done>
</task>
...
print('Auditor SrAO restriction in views:', 'Auditors can only reassign to SrAO or AAO' in view)
print('multi_assign guard intact:', \"role not in ['AG', 'DAG']\" in view)
print('reopen guard intact:', \"role != 'AG'\" in view)
"
```

Expected: All eight checks print True.
</verification>
...
- multi_assign blocked for auditor/clerk via existing `role not in ['AG', 'DAG']` guard
- reopen blocked for auditor/clerk via existing `role != 'AG'` guard
- close of multi-assigned blocked for auditor/clerk via existing `is_multi_assigned` guard
