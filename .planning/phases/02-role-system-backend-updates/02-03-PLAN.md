...
Purpose: Plan 02 handles visibility; this plan handles creation rights and serializer completeness. This plan runs after Plan 02 (Wave 3) because both plans modify backend/records/views.py — sequential execution ensures the create() changes here are applied additively to the file already modified by Plan 02, avoiding overwrite conflicts.
...
@.planning/phases/02-role-system-backend-updates/02-02-SUMMARY.md
@backend/records/serializers.py
@backend/records/views.py
@backend/users/serializers.py
</context>
...
Remove this entire block — `action_required` is now free text with no 'Other' sentinel value (changed in Plan 01). Keep all other validation unchanged.
...
**Confirm existing wiring before editing:**

Before making changes, confirm the following are already present in serializers.py (they are, based on current file read):
- `'section'` is in `MailRecordCreateSerializer.Meta.fields` (confirmed: line 233 shows `'section', 'subsection', 'assigned_to'` in fields list)
- `validate()` already accesses `self.context.get('request')` and `request.user` (confirmed: lines 256-258 show `request = self.context.get('request')` and `if request and request.user: user = request.user`)

No additional wiring is needed — both are already established.
  </action>
  <verify>
```bash
...
# Verify section is in CreateSerializer fields (pre-existing, should pass)
create_fields = list(MailRecordCreateSerializer().fields.keys())
print('section in CreateSerializer:', 'section' in create_fields)
print('CreateSerializer fields:', create_fields)

# Verify 'Other' sentinel validation is gone
import inspect
src = inspect.getsource(MailRecordCreateSerializer.validate)
print('Other sentinel removed:', \"action_required_other\" not in src or 'Other' not in src)
"
```
Expected: First two print True. section confirmed in CreateSerializer. 'Other' sentinel validation absent.
  </verify>
  <done>
MailRecordListSerializer has `attachment_metadata` SerializerMethodField that calls `obj.get_attachment_metadata()`. MailRecordDetailSerializer has the same field. Both serializers include the field in their output. The 'Other' special-case validation is removed from MailRecordCreateSerializer.validate(). Confirmed that `section` was already in CreateSerializer.Meta.fields and validate() was already wired to self.context['request'].user — no additional wiring changes needed.
  </done>
</task>
...
**Read the current state of views.py first** (it was modified by Plan 02 in Wave 2). Apply changes additively — do not overwrite Plan 02's queryset and reassign changes.
...
print('SrAO/AAO/clerk in create:', 'SrAO' in src and 'clerk' in src and 'subsection' in src)
...
# Confirm Plan 02 changes still intact (not overwritten)
print('Plan 02 clerk queryset branch intact:', 'created_by=user' in src)
print('Plan 02 auditor queryset branch intact:', 'auditor_subsections' in src)
print('Plan 02 auditor reassign restriction intact:', 'Auditors can only reassign to SrAO or AAO' in src)
"
```
Expected: First three True (create logic present, old guard gone). Last three True (Plan 02 changes not overwritten).
  </verify>
  <done>
create() allows AG, DAG, SrAO, AAO, clerk, auditor to create mails. Each role's section/subsection is scoped correctly. Unknown roles get 403. Assignee validation in serializer scopes to creator's subsection for non-AG/DAG roles. The old 'Only Accountant General can create mail records.' error message no longer appears. Plan 02's get_queryset() branches and reassign auditor restriction remain intact in views.py.
  </done>
</task>
...
# Verify section wiring confirmed pre-existing
create_fields = list(MailRecordCreateSerializer().fields.keys())
print('section in CreateSerializer (pre-existing):', 'section' in create_fields)
...
# Verify Plan 02 changes still intact
print('Plan 02 auditor queryset intact:', 'auditor_subsections' in src)
print('Plan 02 auditor reassign intact:', 'Auditors can only reassign to SrAO or AAO' in src)
"
```
...
- Plan 02 get_queryset() and reassign changes remain intact in views.py
